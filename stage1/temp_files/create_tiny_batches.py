...
"""
創建微型合併批次
每個合併批次包含 3 個 mini-batch，確保在 token 限制內
"""
...
def create_tiny_combined_batch(start_batch, end_batch, batch_files):
    """創建微型合併批次文件"""
    combined_sql = ""
    batch_count = 0
    ...
    if combined_sql:
        combined_file = f"stage1/output/tiny_batch_{start_batch:04d}_{end_batch:04d}.sql"
        with open(combined_file, 'w', encoding='utf-8') as f:
            f.write(combined_sql)
        
        file_size = len(combined_sql.encode('utf-8'))
        print(f"V 創建微型合併批次: {combined_file}")
...
def main():
    ...
    # 創建微型合併批次（每 3 個批次合併為一個）
    batch_size = 3
    combined_files = []
    
    for start in range(1, total_batches + 1, batch_size):
        end = min(start + batch_size - 1, total_batches)
        combined_file = create_tiny_combined_batch(start, end, batch_files)
        if combined_file:
            combined_files.append(combined_file)
    
    print(f"\nV 創建了 {len(combined_files)} 個微型合併批次文件")
    print("現在可以使用 MCP 工具上傳這些微型合併批次")
    ...
    if len(combined_files) > 10:
        print(f"  ... 還有 {len(combined_files) - 10} 個微型合併批次")
    
    print(f"\n下一步：")
    print("1. 使用 MCP execute_sql 工具上傳這些微型合併批次")
    print("2. 每個微型合併批次包含 3 個原始批次")
